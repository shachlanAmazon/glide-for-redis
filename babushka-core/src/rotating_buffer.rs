        ...
        while prev_position < buffer_len {
            if let Some((request_len, bytes_read)) = u32::decode_var(&buffer[prev_position..]) {
                ...
                if (start_pos + request_len as usize) > buffer_len {
                    ...
                    match T::decode(&buffer[start_pos..start_pos + request_len as usize]) {
                        Ok(request) => {
...
mod tests {
    ...
    fn create_command_request(
        ...
        let args = if args_pointer {
            Some(command::Args::ArgsVecPointer(
                Box::leak(Box::new(args)) as *mut Vec<String> as u64,
            ))
        } else {
            let args_array = command::ArgsArray { args };
            Some(command::Args::ArgsArray(args_array))
        };
        let command = Command {
            request_type: request_type.into(),
            args,
        };
        let command = Some(redis_request::Command::SingleCommand(command));
    ...
    fn assert_request(
        ...
        assert_eq!(command.request_type, expected_type as i32);
        let args: Vec<String> = if args_pointer {
            match command.args.clone().unwrap() {
                command::Args::ArgsArray(_) => unreachable!(),
                command::Args::ArgsVecPointer(ptr) => {
                    *unsafe { Box::from_raw(ptr as *mut Vec<String>) }
                }
            }
        } else {
            match command.args.clone().unwrap() {
                command::Args::ArgsArray(args) => args.args,
                command::Args::ArgsVecPointer(_) => unreachable!(),
            }
        };
