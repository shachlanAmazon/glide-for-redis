...
mod socket_listener {
    ...
    fn send_address(address: String, socket: &mut SocketLike) {
        ...
        block_on_all(socket.write(&buffer)).unwrap();
        buffer.clear();
        let size = block_on_all(socket.read(&mut buffer)).unwrap();
    ...
    fn setup_test_basics() -> TestBasics {
        ...
        start_listener(move |res| {
            let socket = res.expect("Failed to initialize the socket listener");
